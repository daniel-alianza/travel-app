...
export function AppFooter({
  ...
  return (
    <footer
      className={cn(
        "relative z-10 shrink-0 overflow-hidden border-t border-border/60 transition-all duration-1000",
        "bg-linear-to-b from-muted/50 via-muted/30 to-background",
        "shadow-[inset_0_1px_0_0_oklch(1_0_0/0.5)] dark:shadow-[inset_0_1px_0_0_oklch(1_0_0/0.06)]",
    ...
    >
      <div
        ...
        aria-hidden
      />
      <div className="pointer-events-none absolute top-1/2 -left-24 h-48 w-48 -translate-y-1/2 rounded-full bg-primary/6 blur-3xl dark:bg-primary/9" />
        ...
        <div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-start lg:gap-14 xl:gap-20">
          <div className="flex min-w-0 flex-col gap-8">
              ...
              {FOOTER_PARTNER_LOGOS.map((item) => (
                ...
                >
                  <img
                    src={item.src}
                    alt={item.alt}
                    className="max-h-9 w-auto max-w-[min(100%,9rem)] object-contain object-center opacity-90 transition duration-300 hover:opacity-100 sm:max-h-12 sm:max-w-none md:max-h-13"
                  />
                </div>
              ))}
            </div>
            ...
            <div className="max-w-xl space-y-1.5 border-l-2 border-primary/25 pl-4">
              <p className="text-sm leading-snug font-medium text-foreground/95">
          ...
          >
            <div className="flex w-full max-w-md flex-col items-center gap-2 lg:max-w-none lg:items-end">
              <p className="text-center text-[10px] font-semibold tracking-[0.22em] text-muted-foreground uppercase lg:text-right">
                Enlaces útiles
              </p>
              <span
            ...
            <div className="flex max-w-full min-w-0 flex-row flex-nowrap items-center justify-center gap-3 overflow-x-auto overscroll-x-contain pb-0.5 [-ms-overflow-style:none] [scrollbar-width:none] lg:justify-end [&::-webkit-scrollbar]:hidden">
              <a
              ...
              <a
                href="https://ticket-fg-app.fgelectrical.com/login"
                target="_blank"
                rel="noopener noreferrer"
                className={footerLinkClassName}
              >
