...
const Select = React.forwardRef<HTMLSelectElement, SelectProps>(
  ({ className, children, options, placeholder = "Selecciona una opción", ...props }, ref) => {
    ...
    React.useEffect(() => {
      if (hiddenSelectRef.current && selectedValue !== hiddenSelectRef.current.value) {
              ...
              {parsedOptions.map((option) => (
                <button
                  ...
                  className={cn(
                    ...
                    selectedValue === option.value && "bg-primary/15 text-primary font-medium"
                  )}
                >
                  {option.label}
                </button>
