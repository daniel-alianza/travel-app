...
export function FinancialAuthorizationDashboard(props: {
  ...
  return (
            <>
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 max-w-3xl text-sm text-pretty text-muted-foreground sm:text-base">
                      Viáticos con comprobaciones listas para revisión
                      financiera. Cada tarjeta es una solicitud (puede agrupar
                      varios viajes); usa{" "}
              ...
              <section
                className={`group/cola mb-8 rounded-3xl border border-border/60 bg-card/40 p-5 shadow-lg shadow-black/5 ring-1 ring-violet-500/10 backdrop-blur-sm transition-all duration-700 ease-out hover:shadow-xl hover:shadow-violet-500/10 hover:ring-violet-500/20 sm:p-6 ${
                  mounted
                    ? "translate-y-0 opacity-100"
              ...
              {!cargando && errorCarga === null && haySolicitudes && (
                <section
                  className={`relative z-30 mb-8 rounded-3xl border border-violet-500/25 bg-linear-to-br from-card/80 via-card/60 to-violet-500/5 p-5 shadow-lg shadow-violet-500/10 ring-1 ring-violet-500/15 backdrop-blur-sm transition-all duration-700 hover:shadow-xl hover:shadow-violet-500/15 hover:ring-violet-500/25 sm:p-6 ${
                    mounted
                      ? "translate-y-0 opacity-100"
                      : "translate-y-2 opacity-0"
                  }`}
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="mt-0.5 max-w-xl text-sm text-muted-foreground">
                          Ajusta nombre, correo, rango de monto total
                          comprobado, compañía y área para ver solo lo que
                          necesitas.
                        </p>
                      </div>
                    </div>
                ...
                solicitudesFiltradas.length === 0 && (
                  ...
                  >
                    ...
                    <p className="mt-2 text-sm text-muted-foreground">
                      Prueba ampliar el rango de montos, quitar compañía o área,
                      o limpiar los filtros para ver de nuevo toda la cola.
                    </p>
                    <Button
                      type="button"
                      variant="outline"
              ...
              {!cargando && errorCarga === null && !haySolicitudes && (
                <div className="rounded-3xl border border-dashed border-border/80 bg-muted/20 p-12 text-center transition-all duration-500">
                  ...
                  <p className="mt-2 text-sm text-muted-foreground">
                    Cuando existan solicitudes (una sola por envío, con los
                    viajes que correspondan) ya aprobadas y con todos los
                    movimientos comprobados, aparecerán aquí para revisión
                    financiera.
                  </p>
                </div>
              )}
                ...
                solicitudesFiltradas.length > 0 && (
                  ...
                  >
                    <p className="text-xs text-muted-foreground sm:text-sm">
                      Ordenadas como en la cola. El importe es el total
                      comprobado de la solicitud (todos los viajes).
                    </p>
                    <div className="grid gap-4 lg:grid-cols-2">
                      {solicitudesListadoPagina.map((solicitud) => {
                        ...
                        const nMovs = movs.length
                        const nViajes = solicitud.viajes.length
                        const { salida, regreso } =
                          rangoFechasSolicitud(solicitud)
                        return (
                          <article
                            key={solicitud.id}
                            className="group/card relative flex flex-col overflow-hidden rounded-2xl border border-border/60 bg-card/50 shadow-md shadow-black/4 ring-1 ring-border/40 transition-all duration-300 ease-out hover:-translate-y-1 hover:border-violet-400/45 hover:shadow-2xl hover:shadow-violet-500/15 hover:ring-violet-500/25"
                          >
                            <div
