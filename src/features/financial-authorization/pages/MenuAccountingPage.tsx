...
import { BarChart3, ClipboardCheck, ChevronRight, Handshake } from "lucide-react"
        ...
        <div className="mx-auto w-full max-w-7xl px-4 pt-8 pb-28 sm:px-6 sm:py-12 lg:px-8">
          ...
          {page.indicadoresListos ? (
            <div className="animate-travel-panel-in">
              <MenuAccountingSummaryPanel
                ...
                errorMensaje={page.indicadoresError}
              />
            </div>
          ) : (
          ...
          {page.modulosListos ? (
            <div className="animate-travel-panel-in" style={{ animationDelay: "40ms" }}>
              {page.puedeVerConciliacion ||
              page.puedeVerAutorizacionFinanciera ||
              page.puedeVerResumenGastosComprobados ? (
              <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
                {page.puedeVerResumenGastosComprobados ? (
                <button
                  type="button"
                  onClick={() =>
                    navigate("/menu-accounting/expenses-summary")
                  }
                  className="group relative cursor-pointer rounded-3xl border-2 border-transparent bg-card p-6 text-left shadow-[0_4px_6px_-1px_rgba(0,0,0,0.06),0_2px_4px_-2px_rgba(0,0,0,0.05)] transition-all duration-500 ease-out hover:-translate-y-2 hover:scale-[1.02] hover:border-primary/30 hover:bg-card hover:shadow-[0_25px_50px_-12px_rgba(0,0,0,0.18),0_0_0_1px_rgba(14,165,233,0.12),0_12px_40px_-8px_rgba(14,165,233,0.18)] active:scale-[0.99] active:duration-200 sm:p-7 dark:hover:shadow-[0_25px_50px_-12px_rgba(0,0,0,0.45),0_0_0_1px_rgba(56,189,248,0.15)]"
                >
                  <div className="absolute inset-0 rounded-3xl bg-gradient-to-br from-sky-500 to-cyan-600 opacity-0 blur-2xl transition-opacity duration-700 group-hover:opacity-[0.12]" />

                  <div className="relative mb-5 flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br from-sky-500 to-cyan-600 shadow-lg shadow-sky-500/30 transition-all duration-500 ease-out group-hover:scale-110 group-hover:rotate-6 group-hover:shadow-xl group-hover:shadow-sky-500/40 sm:h-16 sm:w-16">
                    <BarChart3 className="h-7 w-7 text-white transition-transform duration-500 group-hover:scale-110 sm:h-8 sm:w-8" />
                  ...
                  <div className="relative flex items-end justify-between gap-4">
                    <div className="min-w-0 flex-1 space-y-2">
                      ...
                      <p className="text-sm leading-relaxed text-muted-foreground transition-colors duration-500 group-hover:text-muted-foreground">
                        Tablero auditable con filtros, gráficas y detalle por
                        solicitud y solicitante (demo).
                      </p>
                    </div>
                  ...
                  <div className="absolute bottom-0 left-1/2 h-1 w-0 -translate-x-1/2 rounded-full bg-gradient-to-r from-sky-500 to-cyan-600 transition-all duration-700 ease-out group-hover:w-1/2" />
                </button>
                ) : null}
                ...
                <p className="rounded-2xl border border-border/60 bg-muted/30 px-4 py-6 text-center text-sm text-muted-foreground">
                  No tienes permisos para abrir los submódulos de contabilidad.
                </p>
              )}
            </div>
          ) : (
